...
class NewsTitleTranslationTest(unittest.IsolatedAsyncioTestCase):
    ...
    def _make_plugin(self, llm_text, template="{content}"):
        ...
        plugin._cfg = lambda key, default=None: (
            template if key == "llm_prompt" else default
        )
        plugin._llm_prompts = []
    ...
    async def test_llm_structured_response_preserves_original_title_containing_han_characters(
        self,
    ):
        plugin = self._make_plugin("【标题】\n模型改写标题\n【正文】\n已修复问题")
    ...
    async def test_llm_structured_response_allows_blank_lines_and_same_line_values(
        self,
    ):
        response = "\r\n  【标题】：夏季更新\r\n\r\n【正文】:  修复了多人模式问题\r\n  "
        plugin = self._make_plugin(response)
    ...
    async def test_llm_structured_response_accepts_supported_outer_markdown_fences(
        self,
    ):
        items = [self.mod.NewsItem("1", "Summer Update", "url-1", "source", 1, "123")]
        for language in ("", "text", "markdown", "plain", "json", "JSON"):
            with self.subTest(language=language):
                ...
                response = f"\n{fence}\n【标题】\n夏季更新\n【正文】\n修复了多人模式问题\n```\n"
                plugin = self._make_plugin(response)
                sections = await plugin._build_sections_llm(
                    ["123"], {"123": items}, None
                )
                merged = sections[0].updates[0]
                self.assertEqual(merged.title, "夏季更新")
    ...
    async def test_llm_structured_body_preserves_internal_newlines_and_indentation(
        self,
    ):
        response = "【标题】\n夏季更新\n【正文】\n  第一行\n\n    第二行\n"
        plugin = self._make_plugin(response)
    ...
    async def test_llm_structurally_valid_english_title_keeps_body_without_protocol_markers(
        self,
    ):
    ...
    async def test_llm_structured_response_preserves_extension_i_han_original_title(
        self,
    ):
        plugin = self._make_plugin("【标题】\n模型标题\n【正文】\n正文")
        items = [
            self.mod.NewsItem("1", "\U0002ebf0 Update", "url-1", "source", 1, "123")
        ]
    ...
    async def test_llm_structured_response_preserves_extension_j_han_original_title(
        self,
    ):
        plugin = self._make_plugin("【标题】\n模型标题\n【正文】\n正文")
        items = [
            self.mod.NewsItem("1", "\U000323b0 Update", "url-1", "source", 1, "123")
        ]
    ...
    async def test_llm_structured_response_preserves_compatibility_supplement_han_original_title(
        self,
    ):
        plugin = self._make_plugin("【标题】\n模型标题\n【正文】\n正文")
    ...
    async def test_llm_multi_notice_uses_first_title_and_preserves_merged_metadata(
        self,
    ):
        plugin = self._make_plugin("【标题】\n首条公告译文\n【正文】\n合并后的正文")
        items = [
